...
<template>
  <section id="faq" class="scroll-anchor bg-linen py-20 lg:py-28" aria-labelledby="faq-heading">
    <div class="shell grid items-start gap-12 lg:grid-cols-[0.8fr_1.2fr] lg:gap-16">
      <div v-reveal class="reveal">
        <FoldEyebrow number="06" label="FAQ" class="mb-6" />
        <h2 id="faq-heading" class="font-display text-4xl leading-[1.04] lg:text-[46px]">Good to know.</h2>
      ...
      <div v-reveal="80" class="reveal border-b border-[#d9d1c1]">
        <div
        ...
        >
          <h3 class="m-0">
            <button
              ...
              @click="toggle(i)"
            >
              <span class="font-display text-xl text-ink">{{ item.q }}</span>
              <span class="faq-marker font-display text-[22px] leading-none text-gold" aria-hidden="true">
                {{ open[i] ? "−" : "+" }}
              </span>
            </button>
          </h3>
          ...
          >
            <div class="faq-panel__inner">
              <p class="max-w-xl pb-6 pr-6 text-[15px] leading-relaxed text-muted">{{ item.a }}</p>
            </div>
          </div>
        </div>
      </div>
